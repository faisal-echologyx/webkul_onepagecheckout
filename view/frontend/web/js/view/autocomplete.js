...
define([
    'jquery',
    'uiComponent',
    'Webkul_OneStepCheckout/js/google_maps_loader',
    'uiRegistry'
], function (
    $,
    Component,
        ...
        setTimeout(function () {
            if(!googleMapError) {
                if (enabled == '1') {
                    ...
                    var geolocationButton = $('#detect_location');
                    var street = $('#' + domID);
                    ...
                    street.each(function () {
                        ...
                        // var countries = window.checkoutConfig.opc_autocomplete.restict_countries;
                        // autocomplete.setComponentRestrictions({'country': countries.split(',')});
                        // autocomplete.setFields(['address_component']);
                        autocomplete.addListener('place_changed', fillInAddress);

                    });
                    $('#' + domID).focus(geolocate);
                    // geolocationButton.on('click', geolocate);
                }
            }
        }, 5000);

    }).fail(function () {
        console.info("Google autocomplete not available");
    });
            ...
            for (var i = 0; i < place.address_components.length; i++) {
                ...
                if (componentForm[addressType]) {
                    ...
                    if (addressType == 'subpremise') {
                        ...
                        street[1] = value;
                    } else if (addressType == 'locality' && street.length == 0) {
                        street[1] = value;
    ...
    geolocate = function () {
        if (navigator.geolocation && window.checkoutConfig.opc_autocomplete.use_geolocation === '1') {
